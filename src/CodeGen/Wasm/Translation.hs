...
localEntries =
    let step :: M.Map T.Type (S.Set Int) -> T.Inst -> M.Map T.Type (S.Set Int)
        step groups (T.Assignment (T.None (T.Subs (T.Local n t))) _) =
            M.insert t (S.insert n existing) groups
          where
            existing = M.findWithDefault S.empty t groups
        step _ (T.Assignment{}) = undefined
...
inst (T.Assignment (T.None (T.Subs lhs)) rhs) = do
    expr rhs
    idx <- getLocalIdx lhs
    appendInst $ BasicInst $ SetLocal idx
inst (T.Assignment{}) = undefined
