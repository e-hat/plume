...
genGlobalTree (DefFn i ps _ e, _) = do
  ...
  appendInst Ret
-- no other declaration types can be global 
genGlobalTree _ = undefined
...
genDecl (IfDecl ic ie eifs me, _) =
  let genCE :: String -> [(ExprAug SymData, DeclAug SymData)] -> State GState ()
      ...
      genCE exit ((cond, body) : rest) = do
        ...
        genCE exit rest
      genCE _ [] = undefined -- if we are here, something impossible has happened
   in do
        exit <- getNextLabel
...
moveExprIntoUnsafe t b@(BinOp Leq _ _, _) = moveRelInto t b
moveExprIntoUnsafe t b@(BinOp Less _ _, _) = moveRelInto t b
moveExprIntoUnsafe t b@(BinOp Geq _ _, _) = moveRelInto t b
moveExprIntoUnsafe t b@(BinOp Greater _ _, _) = moveRelInto t b
moveExprIntoUnsafe t b@(BinOp Equal _ _, _) = moveRelInto t b
moveExprIntoUnsafe t b@(BinOp NotEqual _ _, _) = moveRelInto t b
...
genConditional cond body failure
  | isRel cond =
...
binOpMapping _ = undefined -- this is only for binary operation expressions
...
relOpInverseMapping _ = undefined -- this is only for rel op expressions
